...
class Shopping extends Component {
    ...
    checkAll = () => {
        const checked = this.state.checkedAll;
        const datas = this.state.data;
        let price = 0;
        if (!checked) {
            datas.map(i => {
    ...
    remove = () => {
        const datas = this.state.data;
        const cheked = this.state.checkedAll;
        // 全选时删除
        let check;
        if(cheked) {
            check = false;
            ...
            <div>
                ...
                <Flex className={styles.footer}>
                    <Flex.Item className={styles.btn}>
                        <CheckboxItem style={{ backgroundColor: '#fff' }} checked={this.state.checkedAll} onChange={this.checkAll}>
                            全选
                        </CheckboxItem>
                    </Flex.Item>
